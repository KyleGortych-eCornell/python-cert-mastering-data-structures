...
def mono(image, sepia=False):
    ...
    """
    # Change sepia precondition to return True when the function is implemented

    #assert isinstance(image, RGB)
    ...
    for row in range(height):
        for col in range(width):
            pixel = image[row][col]
            brightness = (0.3 * pixel.red + 0.6 * pixel.green + 0.1 * 
                    pixel.blue)
            if sepia is True:
