...
def row_sums(table):
    ...
    #         all(isinstance(item, (int, float)) for item in table)) #not sure if works for table

    total = [sum(idx) for idx in table]
...
def crossout(table,row,col):
    ...
    # refrence: https://stackoverflow.com/questions/59003986/how-to-remove-a-row-and-column-simultaneously-of-nested-list-in-python-3
    # Farzad Vertigo

    import copy
    c = copy.deepcopy(table)
    if row in range(len(c)):
        del c[row]
    for row in c:
