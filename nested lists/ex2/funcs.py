...
def crossout(table,row,col):
    ...
    """
    # assert (isinstance(table, object?) and
    #         all(isinstance(item, int) for item in table)) #not sure if works for table
    # assert isinstance(row, (int, object?))
    # assert isinstance(column, (int, object?))
    if row in range(len(table)):
        del table[row]
    for row in table:
        if col in range(len(row)):
            del row[col]
...
def place_sums(table):
    ...
    """
    # assert (isinstance(table, object?) and
    #         all(isinstance(item, (int, round(float, 1))) for item in table[1:]))
    # assert isinstance(table[0], str)
    # assert isinstance(table[1:][-1], round(float, 1))
    # or just assert isinstance(table, object) | composition | new class rather than subclass of table?

    table[0].append('Sum')
    for row in table[1:]:
        row.append(round(sum(row), 1)) # table[1:]sum([:])
